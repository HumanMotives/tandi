...
// ——————————————————————————————————————————————
// PUBLIC: Start drone when Play is pressed
// ——————————————————————————————————————————————
let chordLoop, audioUnlocked = false;
async function startDroneLoop() {
  audioUnlocked = true;
...
// DOUBLE‐TAP STATE
// ——————————————————————————————————————————————
let lastTap = 0, tapThreshold = 300; // ms
...
function setup() {
  ...
  // white noise (silent until you twist)
...
function touchMoved(){
  ...
  if (touches.length===1) {
    ...
    noiseVal = foldVal = v;
    // ramp noise gently
    noiseGain.gain.rampTo(foldVal*0.02, 2.0);
  }
  return false;
}
...
function touchEnded(){
  ...
  if (touches.length===0 && initTouches.length===1) {
    const now = millis();
    if (now - lastTap < tapThreshold) {
      // double-tap => place stone
      if (stones.length>=5) {
...
function playChord(time){
  ...
  let cutoff = lerp(400,2000,1-bulgeVal);
  ...
  let wet = constrain(bulgeVal*0.4 + foldVal*0.6, 0, 1);
