...
let synthPure, synthRich, filterNode, tremoloNode, reverbNode, chordLoop, stoneSynth;
...
function setup() {
  ...
  // FX chain: synths → tremolo → filter → reverb → out
  tremoloNode  = new Tone.Tremolo(5, 0).start().toDestination(); 
  filterNode   = new Tone.Filter(800, 'lowpass').connect(tremoloNode);
...
function playChord(time) {
  ...
  // inverted: thick=dark, thin=bright
  const cutoff = lerp(400, 2000, (1 - bulgeVal));
  filterNode.frequency.value = cutoff;

  // use foldVal for tremolo depth & rate
  tremoloNode.depth = foldVal * 0.8;             // depth 0→0.8
  tremoloNode.frequency.value = lerp(0.5, 8, foldVal);
