...
let synthPure, synthRich,
    filterNode, distortionNode, reverbNode,
    chordLoop, stoneSynth;

// **NEW** white-noise nodes:
...
function setup() {
  ...
  // **ADD: noise → lowpass → gain → reverb (silent at first)**
...
function touchMoved() {
  if (touches.length===1) {
    ...
    // **SMOOTH noise fade** over 0.5s up to 2%
...
// —————————————————————————————————————————————————————————————
// DRONE CALLBACK (updated reverb + noise, no distortion)
// —————————————————————————————————————————————————————————————
function playChord(time) {
  ...
  filterNode.frequency.value = lerp(maxCut,minCut,bulgeVal);

  // no more distortion; instead:
  noiseGain.gain.rampTo(foldVal * 0.02, 0.5);
