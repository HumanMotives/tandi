...
let synthPure, synthRich, filterNode, distortionNode, reverbNode, chordLoop, stoneSynth;
...
function setup() {
  ...
  // visual band ≈20% of height
  const regionH = height * 0.2;
  rowSpacing = regionH / (stripes - 1);
  waveAmp = rowSpacing * 0.5;
  stroke('#fff');

  // give ourselves overall headroom
  Tone.Destination.volume.value = -12;

  // FILTER -> DISTORTION -> REVERB -> OUTPUT
...
function draw() {
  ...
  for (let i = 0; i < stripes; i++) {
    ...
    // render each segment with bulge stroke‐weight
...
function playChord(time) {
  ...
  // **now driven by bulgeVal (thick=dull, thin=bright)**
  const minCut = 200, maxCut = 4000;
  const cutoff = lerp(maxCut, minCut, bulgeVal);
  filterNode.frequency.value = cutoff;

  // **foldVal drives audible distortion**
  distortionNode.distortion = foldVal * 1.0;
