...
let freqVal = 2, noiseVal = 0, foldVal = 0, bulgeVal = 0;
...
function draw() {
  ...
  for (let i = 0; i < stripes; i++) {
    ...
    for (let x=0; x<=width; x+=5) {
      ...
      // apply stone repulsion with dynamic burst
      let yy = y0 + y;
      for (let st of stones) {
        ...
        if (d < st.r) {
          ...
          let currStr = st.strength;
          if (st.pushStart > 0) {
            let e = (now - st.pushStart) / st.pushDur;
            if (e < 1) {
              let f2 = easeOutQuad(e);
              currStr += st.strength * f2;
            } else {
              st.pushStart = 0;
            }
          }

          y  += sign * push * currStr * bendF;
          yy  = y0 + y;
        }
      }
      pts.push({ x, y: yy });
...
function playChord(time){
  ...
  reverbNode.wet.rampTo(
    constrain(bulgeVal*0.6 + foldVal*0.4,0,0.9),
    0.5
  );
