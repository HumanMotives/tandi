...
function setup() {
  ...
  noFill();
  strokeCap(ROUND);
  // initial visual band = 20% of height
  ...
  limiterNode = new Tone.Limiter(-1)       // ceiling at –1 dB
                      .set({ release: 0.1 })
                      .toDestination();

  // connect filter → reverb → limiter → speakers
...
function touchEnded() {
  if (touches.length === 0 && initTouches.length === 1) {
    ...
    // pluck a high note
    let note = Tone.Frequency(scaleNotes[scaleNotes.length-1]).transpose(12).toNote();
