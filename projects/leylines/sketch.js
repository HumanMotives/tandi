...
let synthPure, synthRich, filterNode, reverbNode, chordLoop;
let pluckSynth;
...
function touchStarted() {
  if (!audioStarted) {
    // unlock & start transport + chord loop
    Tone.start().then(() => {
...
function touchEnded() {
  if (touches.length===0 && initTouches.length===1) {
    ...
    const st = {
      ...
      r: width*0.15,
      strength: 40,
      // compute loop interval by inverse of freqVal:
      // map freqVal [0.5→5] to interval [2000ms→200ms]
      interval: map(freqVal,0.5,5,2,0.2)
    };
    stones.push(st);

    // schedule its melodic loop
    st.loop = new Tone.Loop(time => {
      // map stone.x → scale degree
...
function playChord(time) {
  ...
  // map rowSpacing [minSp→maxSp] → cutoff [200→2000]
  const minSp = (height*0.15)/(stripes-1),
        maxSp = (height*0.25)/(stripes-1);
  const cutoff = constrain(map(rowSpacing, minSp, maxSp, 200,2000), 200,8000);
