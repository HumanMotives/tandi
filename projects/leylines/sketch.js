...
function draw() {
  background(245);
  // time‐warp: slow when pinch out, fast when pinch in
  ...
  for (let i = 0; i < stripes; i++) {
    ...
    beginShape();
      for (let x = 0; x <= width; x += 5) {
        ...
        // Perlin warp
        y += (noise(x * noiseVisual * 0.1 + t * 0.5, i * 0.2) - 0.5)
             * amplitude * 0.5;
        // wave-fold
        if (foldVisual > 0) {
...
function touchMoved() {
  if (dragging && touches.length === 2) {
    ...
    // region for audio mods
    const cx     = (touches[0].x + touches[1].x) / 2;
    const region = cx < width/3 ? 'A'
                  : cx < 2*width/3 ? 'B' : 'C';
    // inward pinch → val→1, outward→val→0
    let val = constrain(map(scaleFactor, 1, 3, 1, 0), 0, 1);
    modSynth(region, val);

    // visuals only on region B
    if (region === 'B') {
      noiseVisual = val;
      foldVisual  = val;
    }
